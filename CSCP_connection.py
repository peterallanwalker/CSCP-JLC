            ...
            if data:
                ...
                if messages:
                    for msg in messages:
                        ...
                        # HACK TO FILTER MESSAGES
                        # WILL NEED TO CHANGE THIS TO RECEIVE DIFFERENT MESSAGE TYPES
                        # OR FOR DIFFERENT STRIP MAPPING

                        # ONLY RECEIVE MESSAGES RELATING TO FIRST 8 STRIPS
                        #if msg.strip in range(8):
                        #    self.messages.append(parse.Message(msg))
                        # DOH above doesnt work, parse the message first!
                        self.messages.append(parse.Message(msg))
    ...
    def send(self, msg):
        ...
        if self.status == "Connected":
            try:
                ...
                pass
            #except OSError:
            #    print("CSCP_connection.send - OS error!")
...
if __name__ == '__main__':
    ...
    while True:
        ...
        #if message and message.operation == "read_fader_label" and message.strip in range(8):
        if message and message.strip in range(8):
