...
# - V0.8b, DB report that the app often crashes when memories are loaded on their calrec mixer.
# - so they have a script monitoring if its running that will auto restart it when its not
# - They have asked me to remove the connection settings prompt at startup so it will start up and connect without
# - having to OK the settings. -
# -    THIS IS WHAT I'M DOING FOR THIS VERSION
# - They've also asked me to look at fixing the crash, might need to do proper logging to figure that out as I don't hace
# - the hardware.
...
if __name__ == '__main__':
    ...
    # settings = config.get_settings()  # - this attempts to load and prompts user for confirm/change
    settings = config.load_settings()

    # Save connection settings for next start up - dont need to do this now as not changing the settings
